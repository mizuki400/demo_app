...
class Customers(models.Model):
    ...
    # DBのカラムに相当する部分の定義
    id = models.AutoField(primary_key=True) # primary_key : 重複が許されないよ
    last_name = models.CharField(max_length=30)
    first_name = models.CharField(max_length=30)
    limit_balance = models.IntegerField(default = 100000)
    sex = models.IntegerField(choices = gender_optinons,default=1)
    ...
    def __str__(self):
        if self.proba == 0.0:
            ...
            return '%s, %d ,%s ,%d, %s , %s' % (self.registered_date.strtime('%Y-%m-%d'), self.id , self.last_name+self.first_name, self.result, '{}%'.format(round(self.proba, 2)) , self.comment)
